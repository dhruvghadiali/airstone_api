...
/**
 * Signs an employee in and issues the JWT their later requests carry.
 *
 * Public because this is one of the endpoints that hands a token out rather
 * than asking for one.
 *
 * The user type is fixed to `user_type.EMPLOYEE` here and never read from the
 * request, so a super admin or an admin must use their own signin endpoint.
 * An account must also be active; one that was deactivated cannot sign back
 ...
 *
 * @route   POST /employee/auth/signin
 * @access  Public
 *
 * @param   {import("express").Request} req
 * @param   {Object} req.body Validated by `employee_signin_schema`, which rejects
 *                            unknown fields.
 ...
 *
 * @throws  {app_error} 401 `INVALID_CREDENTIALS` when no active employee
 *                      matches the username, or the password is wrong.
 */
const employee_signin = async (req, res) => {
  const user = await authenticate_by_user_type(req.body, user_type.EMPLOYEE);

  return send_response(
    res,
    http_status.OK,
    user_messages.EMPLOYEE_SIGNED_IN,
    build_signin_payload(user),
  );
};

module.exports = employee_signin;
