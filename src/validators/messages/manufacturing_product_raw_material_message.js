 ...
 * manufacturing product helper checks it and this is what it reports.
 *
 * `NOT_FOUND` and `INVALID_ID` address one line inside a product. Each entry
 * carries its own `_id`, so an endpoint can update or remove a single line
 * without rewriting the list.
 */
const manufacturing_product_raw_material_messages = Object.freeze({
  ADDED: "Raw material added successfully",
  UPDATED: "Raw material updated successfully",
  DELETED: "Raw material deleted successfully",
  NOT_FOUND: "Raw material not found on this product",
  INVALID_ID: "Invalid raw material line id",
});

const manufacturing_product_raw_material_validation_messages = Object.freeze({
  ...
  QTY_MIN: `Quantity must be at least ${manufacturing_product_raw_material_validation_limits.QTY_MIN}`,
  QTY_MAX: `Quantity must not exceed ${manufacturing_product_raw_material_validation_limits.QTY_MAX}`,
